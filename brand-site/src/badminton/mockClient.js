...
export const mockClient = {
  ...
  async listLinkUserInviteMatches(notificationId, { kind = "singles", limit = 20, pageToken = null } = {}) {
    ...
    for (const m of pageItems) {
      for (const id of [...(m.teamA || []), ...(m.teamB || [])]) playerIds.add(id);
    }
    const players = (db.participants || [])
      .filter(p => playerIds.has(p.id) || playerIds.has(p.userId))
      .map(p => ({
        id: p.id,
        name: p.name,
    ...
    const result = { items: pageItems, pageToken: nextToken, players };
