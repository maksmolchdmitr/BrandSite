<template>
  <div class="page">
    <div class="content">
      <div class="topRow">
        <h1 class="title">{{ $t('badminton.ratings.doublesHistoryTitle') }}</h1>
      </div>

      <BadmintonHubCtaRow current="doubles-rating-history" :disabled="loading" @logout="handleLogout" />
      ...
      <div class="card">
        <div class="cardHeader">
          <div>
            <div class="cardTitle">{{ $t('badminton.ratings.doublesHistoryChartTitle') }}</div>
        ...
        <div v-if="stats" class="statsRow">
          <div class="stat">
            <div class="statLabel">{{ $t('badminton.ratings.doublesHistoryPairs') }}</div>
            <div class="statValue muted">{{ stats.pairs }}</div>
          </div>
          <div class="stat">
          ...
          <div class="stat">
            <div class="statLabel">{{ $t('badminton.ratings.historyPending') }}</div>
            <div class="statValue muted">{{ stats.pending }}</div>
          </div>
        </div>
        ...
        <DoublesRatingHistoryChart
          ...
          :empty-text="$t('badminton.ratings.doublesHistoryEmpty')"
...
import DoublesRatingHistoryChart from "@/components/badminton/DoublesRatingHistoryChart.vue";
import LoadingPhrase from "@/components/LoadingPhrase.vue";
import { badmintonClient } from "@/badminton/client.js";
import { DOUBLES_RATING_HISTORY_SAFETY_CAP } from "@/badminton/ratingHistory.js";
...
export default defineComponent({
  components: { BadmintonHubCtaRow, DoublesRatingHistoryChart, LoadingPhrase },
  data() {
    return {
      loading: false,
      error: "",
  ...
  computed: {
    ...
    historySafetyCap() {
      return DOUBLES_RATING_HISTORY_SAFETY_CAP;
    },
    historyTruncated() {
    ...
    stats() {
      ...
      const pairs = new Set(points.map((point) => point.teamId).filter(Boolean)).size;
      const pending = points.filter((point) => !isRated(point)).length;
  ...
  methods: {
    ...
    async loadHistory() {
      ...
      this.error = "";
      try {
        const page = await badmintonClient.listMyDoublesRatingHistory({
